...
function MedicinePrescription({ medicines, selectedMedicines, setSelectedMedicines }) {
    const addMedicine = () => {
        setSelectedMedicines([...selectedMedicines, { ma_thuoc: '', so_luong: 1, ghi_chu: '' }]);
    };
                ...
                {selectedMedicines.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {selectedMedicines.map((med, index) => {
                            const medInfo = medicines.find(m => m.ma_thuoc === med.ma_thuoc);
                            return (
                                            ...
                                            >
                                                ...
                                                {medicines.map(m => (
                                                    <option key={m.ma_thuoc} value={m.ma_thuoc}>
                                                        {m.ten_thuoc} ({m.so_luong_thuoc_ton_thuoc} tồn kho)
                                                    </option>
                                                ))}
                                            </select>
                                        </div>
